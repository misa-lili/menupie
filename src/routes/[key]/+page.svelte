<script lang="ts">
  ...
  export let data: { body: Menu; token: GoogleTokenPayload }
  ...
  const select = (
    event,
    parameters: { type: string; gidx?: number; idx?: number; data: any },
  ) => {
    selected = parameters
    ...
    menu = {
      ...
      footers: data.body.footers.map((footer) => {
        if (typeof footer === "string")
          return { id: crypto.randomUUID(), value: footer }
  ...
  const addGroup = () => {
    menu.groups = [
      ...menu.groups,
      { name: "", id: crypto.randomUUID(), items: [] },
    ]
    relayout()
  }
  ...
  const signInAndSave = async () => {
    const result = await fetch(
      `/api/v1/users?key=${email}&pw=${password}`,
    ).then((r) => r.json())
...
<div class="_template">
  ...
  <div class="_wrapper_title">
    <div
      ...
      on:focus={(event) =>
        select(event, { type: "title", idx: 0, data: menu.title })}
      on:input={(event) => {
  ...
  <div class="_wrapper_header">
    {#each menu.headers as header, idx (header.id)}
      <div
        ...
        on:keydown={onKeyDown}
        on:focus={(event) =>
          select(event, { type: "header", idx, data: header })}
        on:input={(event) => {
      ...
      >
        <div class="_group_properties">
          <div
            ...
            on:keydown={onKeyDown}
            on:focus={(event) =>
              select(event, { type: "group", idx: gidx, data: group })}
            on:input={(event) => {
              ...
              >
                <div class="_item_header">
                  <div
                    ...
                    on:input={(event) => {
                      menu.groups[gidx].items[iidx].name =
                  ...
                  <div
                    ...
                    on:input={(event) => {
                      menu.groups[gidx].items[iidx].price =
                        handleInputText(event)
                    }}
                    on:blur={(event) => {
                      placehold(event)
                ...
                <div
                  ...
                  on:input={(event) => {
                    menu.groups[gidx].items[iidx].description =
                      handleInputText(event)
                  }}
                  on:blur={(event) => {
                    placehold(event)
    ...
    <div class="_footer_wrapper">
      {#each menu.footers as footer, idx (footer.id)}
        <div
          ...
          on:keydown={onKeyDown}
          on:focus={(event) =>
            select(event, { type: "footer", idx, data: footer })}
          on:input={(event) => {
